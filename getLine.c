...
ssize_t input_buf(info_t *info, char **buf, size_t *len)
{
	...
	if (!*len) /* if nothing left in the buffer then fills it */
	{
...
ssize_t get_input(info_t *info)
{
	...
	{
		j = i; /* init a new iterator to the current buf position */
	...
	*buf_p = buf;
	return (r); /* returns a length of buffer from _getline() */
}
 ...
 * @info: the parameter struct
 * @ptr: the address of pointer to buffer and preallocated or NULL
...
int _getline(info_t *info, char **ptr, size_t *length)
{
	...
	if (!new_p) /* Malloc Failure in the program */
		return (p ? free(p), -1 : -1);
